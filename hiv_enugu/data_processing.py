...
def load_data(file_path):
    ...
    try:
        ...
        weekly_df.fillna(method='bfill', inplace=True) # For any NaNs at the beginning after ffill
        ...
        if weekly_df['time_idx'].isnull().any():
            if len(weekly_df) == 1:
                ...
                print("Warning: time_idx contains NaNs after calculation. Defaulting to row numbers.")
        ...
        print(f"Error: Missing expected column in CSV: {e}. Ensure 'date', 'number', 'cumulative' exist.")
        return None
