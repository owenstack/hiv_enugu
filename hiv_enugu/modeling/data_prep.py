...
def prepare_data_for_modeling(df, n_splits=5):
    ...
    if np.any(np.isnan(y)) or np.any(np.isinf(y)):
        ...
        y_series.fillna(method='ffill', inplace=True)
        y_series.fillna(method='bfill', inplace=True) # For leading NaNs
        y = y_series.values
        if np.any(np.isnan(y)) or np.any(np.isinf(y)):
             raise ValueError("Invalid values (NaN or Inf) detected in target variable 'cumulative' even after attempting ffill/bfill.")

    if len(y) < 52:  # At least 1 year of weekly data
        print(f"Warning: Insufficient data points for modeling (found {len(y)}, expected at least 52). Model performance may be affected.")
        # Reducing n_splits if data is very short
        if len(y) < 10 and n_splits > 2 : # Arbitrary small number
            n_splits = 2
        elif n_splits > len(y) // 4 and len(y) // 4 > 1: # Ensure at least 4 data points per split if possible
    ...
    if n_splits >= len(X) :
        print(f"Warning: n_splits ({n_splits}) is >= number of samples ({len(X)}). Adjusting n_splits.")
        n_splits = max(2, len(X) -1) # Ensure n_splits is at least 2 if possible, and less than n_samples
    ...
    if min_samples_for_split < min_required_data_per_split and n_splits > 2:
        ...
        print(f"Adjusted n_splits to {n_splits} to ensure at least {min_required_data_per_split} samples per split.")
    ...
    if n_splits < 2:
        print(f"Warning: n_splits is {n_splits}. TimeSeriesSplit requires n_splits >= 2. Setting to 2.")
        n_splits = 2
    ...
    try:
        ...
        raise ValueError(f"Error during TimeSeriesSplit: {e}. n_splits={n_splits}, X length={len(X)}")
