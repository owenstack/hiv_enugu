...
def build_ensemble_models(X, y, fitted_models, model_metrics, cv_splits):
    ...
    ensemble_models["Simple Average"] = {
        "predict": lambda x_new: np.mean(
            np.column_stack([m["function"](x_new, *m["parameters"]) for m in fitted_models.values()]),
            axis=1,
        ),
        "type": "simple_average"
    }
    ...
    weights_r2 = np.array([model_metrics[name].get("test_r2", 0) for name in model_names_in_order])
    weights_r2 = np.maximum(weights_r2, 0) # Ensure non-negative weights (R2 can be negative)
    if weights_r2.sum() > 0:
        weights_r2 /= weights_r2.sum()
    else:
        weights_r2 = np.ones(len(model_names_in_order)) / len(model_names_in_order)

    for i, name in enumerate(model_names_in_order):
        if name not in global_model_weights_for_reporting: global_model_weights_for_reporting[name] = {}
    ...
    ensemble_models["Weighted Average (R2)"] = {
        "predict": lambda x_new: np.sum(
            np.column_stack([fitted_models[name]["function"](x_new, *fitted_models[name]["parameters"]) for name in model_names_in_order])
    ...
    mse_values = np.array([model_metrics[name].get("test_rmse", np.inf)**2 for name in model_names_in_order])
    ...
    if weights_inv_mse.sum() > 0:
        ...
        weights_inv_mse = np.ones(len(model_names_in_order)) / len(model_names_in_order)
    ...
    weighted_avg_pred_inv_mse = np.sum(base_features_array * weights_inv_mse.reshape(1, -1), axis=1)
    ...
    ensemble_models["Weighted Average (R2)"]["component_weights"] = global_model_weights_for_reporting
    ensemble_models["Weighted Average (InvMSE)"]["component_weights"] = global_model_weights_for_reporting
    ...
    feature_names = list(ml_features.columns) # Get feature names from the DataFrame returned by create_ml_features
    ...
    for name, model_obj in [("Random Forest", best_rf), ("Gradient Boosting", best_gb)]:
        ...
        predictor = make_ml_predictor(model_obj, scaler, X.min(), X.max()) # X here is the original time_idx
        y_pred_on_full_X_for_metrics = model_obj.predict(ml_features_scaled) # Predict on the same features used for training for overall metrics
