...
def fit_growth_models(X, y, cv_splits):
    ...
    models = {
        "Exponential": (
            exponential_model,
            (
                [np.max(y) / 2 if np.max(y) > 0 else 1, 0.005, np.min(y) if len(y) > 0 else 0],
                [0, 0.0001, -np.inf],
        ...
        "Logistic": (
            ...
            (
                [
                    ...
                    0.01,
                    np.median(X) if len(X) > 0 else 0.5,
        ...
        "Richards": (
            ...
            (
                ...
                [
                    np.max(y) if np.max(y) > 0 else 1,
                    0.001,
                    X.min() if len(X) > 0 else 0,
                    0.1,
                    -np.inf,
                ],
                [
                    np.max(y) * 2 if np.max(y) > 0 else 2,
                    0.1,
                    X.max() if len(X) > 0 else 1,
                    10,
                    np.inf,
                ],
            ),
    ...
    for name, (model_func, bounds_config) in models.items():
        ...
        try:
            ...
            try:
                ...
                print(
                    f"  Warning: Initial fit on full data failed for {name}. Using default p0 for CV."
                )
                initial_params_for_cv = p0_initial
            except ValueError as ve:
                print(
                    f"  ValueError during initial fit for {name}: {ve}. Using default p0 for CV."
                )
                initial_params_for_cv = p0_initial
            ...
            if not cv_splits:  # Ensure cv_splits is not empty
                ...
                try:
                    ...
                    plot_residuals(
                        ...
                        y_pred_full,
                        name,
                        filename_suffix="_full_data_no_cv",
                        filename=f"residuals_{name}_full_data_no_cv.png",
                    )
                    plot_qq(
                        residuals_full,
                        name,
                        filename_suffix="_full_data_no_cv",
                        filename=f"qq_plot_{name}_full_data_no_cv.png",
                    )
                    plot_residuals_histogram(
                        residuals_full,
                        name,
                        filename_suffix="_full_data_no_cv",
                        filename=f"residuals_histogram_{name}_full_data_no_cv.png",
                    )
                    ...
                    print(
                        f"{name} Model (No CV) Metrics: Train RMSE: {model_metrics[name]['train_rmse']:.2f}, Train R2: {model_metrics[name]['train_r2']:.4f}"
                    )
            ...
            for i, (train_index, test_index) in enumerate(cv_splits):
                ...
                if len(X_train) < len(initial_params_for_cv) or len(y_train) < len(
                    initial_params_for_cv
                ):
                    print(
                ...
                if (
                    current_fold_test_r2 > best_overall_test_r2
                ):  # Check if this split's popt is the best overall
            ...
            if best_popt_overall is None:  # If all splits failed or no better popt found
                best_popt_overall = (
                    initial_params_for_cv  # Fallback to initial parameters from full data fit
                )
            ...
            final_popt, _ = curve_fit(
                ...
                xtol=1e-7,
                gtol=1e-7,
            )  # Stricter tolerance for final fit
