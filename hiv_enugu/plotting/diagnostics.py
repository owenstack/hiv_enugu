...
@plot_manager
def plot_residuals(
    y_true, y_pred, model_name, filename_suffix="", **kwargs
):  # Added filename_suffix and **kwargs
...
@plot_manager
def plot_qq(
    residuals, model_name, filename_suffix="", **kwargs
):  # Added filename_suffix and **kwargs
    """Create a Q-Q plot of residuals."""
    fig = plt.figure(figsize=(8, 8))  # Create a new figure
    stats.probplot(residuals, dist="norm", plot=plt)
    plt.title(
        f"Q-Q Plot of Residuals for {model_name}{filename_suffix}"
    )  # Include suffix in title
...
def plot_residuals_histogram(
    residuals, model_name, filename_suffix="", **kwargs
):  # Changed y_true, y_pred to residuals, added filename_suffix and **kwargs
    ...
    plt.title(
        f"Histogram of Residuals for {model_name}{filename_suffix}"
    )  # Include suffix in title
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    # Filename is passed via kwargs to plot_manager
