...
# Assuming utils.py contains generate_bootstrap_predictions and is in parent directory of plotting
# Adjust if generate_bootstrap_predictions is elsewhere, e.g., hiv_enugu.utils
try:
    from ..utils import generate_bootstrap_predictions
except ImportError:
    # Fallback for environments where ..utils might not be directly accessible during dev
    # This assumes a placeholder or direct definition if hiv_enugu.utils is not structured as expected
    def generate_bootstrap_predictions(model_func, x_future, params, n_samples=100):
        print(f"Warning: Using placeholder generate_bootstrap_predictions. n_samples={n_samples}")
        # Simple placeholder: return mean and some arbitrary spread
        mean_pred = model_func(x_future, *params)
        spread = 0.1 * mean_pred
        return mean_pred, mean_pred - spread, mean_pred + spread
...
@plot_manager
def visualize_model_fits(df, X, y, final_train_idx, final_test_idx, y_train, y_test, fitted_models, **kwargs): # Matched signature from user's main() call
    ...
    plt.scatter(df['date'].iloc[final_train_idx], y_train, color='blue', alpha=0.6, label='Training Data')
    plt.scatter(df['date'].iloc[final_test_idx], y_test, color='red', alpha=0.6, label='Testing Data')

    colors = ['green', 'purple', 'orange', 'brown', 'teal', 'magenta'] # Added more colors
    for i, (name, model) in enumerate(fitted_models.items()):
        y_pred_full_range = model['function'](X, *model['parameters']) # Predict on the full X range
        plt.plot(df['date'], y_pred_full_range, color=colors[i % len(colors)], linewidth=2, label=f'{name} Model')
...
@plot_manager
def visualize_ensemble_comparison(df, X, y, cv_splits, fitted_models, ensemble_models_dict, best_model_name, **kwargs): # Renamed ensemble_models to ensemble_models_dict
    ...
    if best_model_name in fitted_models:
        ...
        y_pred_best_individual = best_individual_model_details['function'](X, *best_individual_model_details['parameters'])
        plt.plot(df['date'], y_pred_best_individual, color='green', linewidth=2, linestyle='--', label=f'Best Individual ({best_model_name})')
    ...
    for i, name in enumerate(ensemble_models_to_plot):
        if name in ensemble_models_dict:
            ...
            plt.plot(df['date'], y_pred_ensemble, color=colors[i % len(colors)], linewidth=2, label=f'{name} Ensemble')
        else:
            print(f"Warning: Ensemble model '{name}' not found in ensemble_models_dict for plotting.")

    plt.title('HIV Growth Ensemble Models Comparison - Enugu State, Nigeria', fontsize=16) # Removed date range, dynamic
    plt.xlabel('Date', fontsize=14)
    plt.ylabel('Cumulative Number of HIV Patients', fontsize=14) # Changed from "Number of HIV Patients"
    plt.legend(fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
...
@plot_manager
def visualize_metrics_comparison(model_metrics, ensemble_metrics_dict, **kwargs): # Renamed ensemble_metrics
    ...
    for model_name, metrics_values in all_metrics_combined.items():
        ...
        if model_name in model_metrics:
            ...
            metrics_data['RMSE'].append(metrics_values.get('test_rmse', np.nan)) # Or just 'rmse' if that's the key
    ...
    metrics_df_copy.dropna(subset=['RMSE', 'R²', 'MAE'], inplace=True) # Drop rows where essential metrics are NaN for scoring
    ...
    if not metrics_df_copy.empty:
        ...
        metrics_df_copy['RMSE_norm'] = (metrics_df_copy['RMSE'] - rmse_min) / (rmse_max - rmse_min + 1e-10)
        metrics_df_copy['MAE_norm'] = (metrics_df_copy['MAE'] - mae_min) / (mae_max - mae_min + 1e-10)
        # For R², higher is better, so (R² - min) / (max - min) gives higher scores for better R²
        metrics_df_copy['R²_norm_score'] = (metrics_df_copy['R²'] - r2_min) / (r2_max - r2_min + 1e-10)

        # Overall score: (1 - RMSE_norm) + R²_norm_score + (1 - MAE_norm)
        # Max score is 3, min is 0.
        metrics_df_copy['Overall_Score'] = (1 - metrics_df_copy['RMSE_norm']) + metrics_df_copy['R²_norm_score'] + (1 - metrics_df_copy['MAE_norm'])
        metrics_df_copy['Overall_Score'] = metrics_df_copy['Overall_Score'] / 3.0 # Normalize to 0-1 range

        metrics_df_copy = metrics_df_copy.sort_values('Overall_Score', ascending=False)
        sns.barplot(x='Model', y='Overall_Score', hue='Type', data=metrics_df_copy, ax=ax4, dodge=True)
    else:
        ax4.text(0.5, 0.5, "Not enough data for Overall Score", horizontalalignment='center', verticalalignment='center')
...
@plot_manager
def create_validation_plot(df, X, y, best_individual_model_details, best_ensemble_model_info, ensemble_name, fitted_models, best_model_name, **kwargs):
    """Create validation plot comparing actual vs predicted values for historical data."""
    # Predictions for best individual model
    historical_best_individual_pred = best_individual_model_details['function'](X, *best_individual_model_details['parameters'])
    ...
    plt.plot(df['date'], y, color='blue', linewidth=2.5, alpha=0.8, label='Actual Cumulative HIV Cases')

    plt.plot(df['date'], historical_best_individual_pred, color='green', linestyle='--',
             linewidth=2, label=f'Best Individual: {best_model_name}')
    plt.plot(df['date'], historical_ensemble_pred, color='purple', linestyle=':',
             linewidth=2, label=f'Best Ensemble: {ensemble_name}')
    ...
    metrics_text = (f"--- Best Individual ({best_model_name}) ---\n"
...
@plot_manager
def forecast_future_trends(df, X, y, best_individual_model_details, best_ensemble_model_info, ensemble_name, fitted_models, best_model_name, generate_bootstrap_predictions_func, **kwargs):
    ...
    future_dates_daily = pd.date_range(start=last_date + pd.Timedelta(days=1), periods=5*365, freq='D')
    ...
    future_time_idx_weekly = np.arange(max_train_time_idx + 1, max_train_time_idx + 1 + num_future_weeks)
    ...
    future_plot_dates_weekly = pd.date_range(start=last_week_date + pd.Timedelta(weeks=1), periods=num_future_weeks, freq='W-SUN') # Or matching df's week frequency
    ...
    best_model_future_pred = best_individual_model_details['function'](future_time_idx_weekly, *best_individual_model_details['parameters'])

    # Predictions for best ensemble model
    # The 'predict' function in best_ensemble_model_info expects X (time_idx)
    best_ensemble_future_pred = best_ensemble_model_info['predict'](future_time_idx_weekly) # Use weekly time_idx
    ...
    plt.plot(df['date'], y, color='blue', alpha=0.8, linewidth=2, label='Historical Data') # Changed from scatter

    # Plot historical fit of the best individual model for context
    historical_fitted_best_individual = best_individual_model_details['function'](X, *best_individual_model_details['parameters'])
    plt.plot(df['date'], historical_fitted_best_individual, color='darkgreen', linestyle='-', linewidth=1.5,
             label=f'Historical Fit ({best_model_name})')

    # Plot forecasts
    plt.plot(future_results_df['date'], future_results_df['best_model_forecast'], color='green', linestyle='--',
             linewidth=2, label=f'Forecast ({best_model_name})')
    plt.plot(future_results_df['date'], future_results_df['best_ensemble_forecast'], color='purple', linestyle=':',
             linewidth=2, label=f'Forecast ({ensemble_name} Ensemble)')

    # Plot confidence interval for the best individual model's forecast
    plt.fill_between(future_results_df['date'],
                     future_results_df['lower_bound_ci'],
                     future_results_df['upper_bound_ci'],
                     color='green', alpha=0.2, label='95% CI (Indiv. Model)')
    ...
    forecast_csv_path = os.path.join(forecast_dir, 'forecast_results_next_5_years.csv') # Modified filename slightly
